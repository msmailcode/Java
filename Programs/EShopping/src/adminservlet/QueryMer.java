...
//进行商品查询
public class QueryMer extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(n == 0 || n == 2){
			...
			request.getRequestDispatcher("/Admin/adminQueryMer.jsp").include(request, response);
			return;
		}else if(n == 3){ //按优惠价来查询
			String sql = "select * from book where saleprice like '%" +qKey+ "%'";
			ArrayList<Book> list = dao.ListAllBookInformation(sql);
			request.setAttribute("list", list);
			request.getRequestDispatcher("/Admin/adminQueryMer.jsp").include(request, response);

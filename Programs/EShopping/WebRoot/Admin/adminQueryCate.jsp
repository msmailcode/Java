					...
					<tr>
						<td height="30" class="blueText" align="center">
							<span class="blueText"><a
								href="/EShopping/servlet/AdminCateList" class="blueText">分类管理</a>
							</span>
							<br>
							<br>
						</td>
									...
									<TR>
										<TD noWrap align=left width=*>
											...
											<INPUT class=button_normal
												onClick="f2();" type=submit value=删除(D)
												name=btnDelete>
											<INPUT class=button_normal onClick="f3()"
												type=submit value="导出(E)" name=btnOutExcel>
										</TD>
							...
							<TABLE class=datagrid_normal id=DataGrid1
								...
								<TBODY>
									<TR class=datagrid_headerstyle_normal align=middle
										nowrap="nowrap">
										<TD noWrap>
											<A href="javascript:dosort('model_name');">序号</A>
										</TD>
										<TD noWrap>
											<A href="javascript:dosort('model_name');">商品分类名称</A>
										</TD>
										<TD noWrap>
											<A href="javascript:dosort('title');">商品分类描述</A>
										</TD>
										<TD noWrap>
											<A href="javascript:dosort('title');">商品分类维护</A>
										</TD>
									</TR>
									<page:pager dz="5">
										<!-- dz代表默认的显示记录条数，如果不指定，则默认为20条 -->
										<c:forEach var="row" items="${requestScope.list}" varStatus="status">
